...
import { Search, Box, ShoppingCart, Gift, ShoppingBag, ArrowLeft, Upload, Check, Crop, Wand2, Sparkles, Save, RotateCcw, Send, Bot, User, Loader2, X, CheckCircle, AlertCircle, History, Clock, Trash2, Plus, Minus, MapPin, CreditCard, Calendar, Monitor, Smartphone } from 'lucide-react';
...
export default function App() {
  ...
  const AppContent = (
    <div className={`min-h-screen bg-orange-50/30 font-sans text-gray-800 ${isMobileSimulator ? 'mobile-mode' : ''}`}>
      {/* --- HEADER --- */}
      <header className="bg-white border-b border-gray-100 py-4 px-6 md:px-12 sticky top-0 z-50 shadow-sm">
        {/* Mobile / Simulator Header Layout */}
        {isMobileSimulator ? (
           <div className="flex flex-col gap-3">
              {/* Top Row: Logo Centered */}
              <div className="flex justify-center items-center relative">
                 <div className="font-bold text-xl cursor-pointer" onClick={() => { setSelectedProduct(null); setCurrentView('home'); }}>
              ...
              <div className="flex items-center justify-center gap-8 text-gray-600">
                  <button 
                    className={`hover:text-orange-500 transition-colors ${!isMobileSimulator ? 'bg-orange-500 text-white p-1.5 rounded-lg' : ''}`}
      ...
      <main>
        {currentView === 'customizer' && selectedProduct ? (
          ...
          <LandingPage onProductClick={handleProductClick} isMobile={isMobileSimulator} />
        )}
      </main>
...
function MobileSimulatorWrapper({ children }: { children?: React.ReactNode }) {
  return (
    <div className="fixed inset-0 bg-gray-900 flex items-center justify-center p-4 z-[1000] overflow-hidden">
      {/* Phone Frame */}
      <div 
        className="relative bg-black rounded-[40px] shadow-2xl border-[8px] border-gray-800 overflow-hidden mx-auto transition-all duration-300"
        style={{ 
            width: '375px', 
            height: '85vh', 
            maxHeight: '850px',
            boxShadow: '0 0 0 2px #333, 0 20px 40px rgba(0,0,0,0.4)'
        }}
      >
        {/* Notch / Dynamic Island */}
        <div className="absolute top-2 left-1/2 transform -translate-x-1/2 w-28 h-7 bg-black rounded-full z-50 flex items-center justify-center">
            {/* Camera dot */}
            <div className="w-2 h-2 rounded-full bg-gray-900 mr-4"></div>
             {/* Sensor dot */}
            <div className="w-1.5 h-1.5 rounded-full bg-gray-800"></div>
        </div>

        {/* Status Bar Mockup */}
        <div className="absolute top-0 left-0 right-0 h-10 bg-white z-40 flex justify-between items-center px-6 pt-2">
            <span className="text-[10px] font-bold text-gray-900">9:41</span>
            <div className="flex gap-1.5">
                 <div className="w-3 h-3 text-gray-900"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg></div>
                 <div className="w-3 h-3 text-gray-900"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M15.67 4H14V2h-4v2H8.33C7.6 4 7 4.6 7 5.33v15.33C7 21.4 7.6 22 8.33 22h7.33c.74 0 1.34-.6 1.34-1.33V5.33C17 4.6 16.4 4 15.67 4z"/></svg></div>
            </div>
        </div>

        {/* Side Buttons (Visual only) */}
        <div className="absolute top-24 -left-2.5 w-1 h-8 bg-gray-700 rounded-l-md"></div> {/* Mute */}
        <div className="absolute top-36 -left-2.5 w-1 h-14 bg-gray-700 rounded-l-md"></div> {/* Vol Up */}
        <div className="absolute top-52 -left-2.5 w-1 h-14 bg-gray-700 rounded-l-md"></div> {/* Vol Down */}
        <div className="absolute top-40 -right-2.5 w-1 h-20 bg-gray-700 rounded-r-md"></div> {/* Power */}
        ...
        {/* Home Indicator */}
        <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gray-900 rounded-full z-50"></div>
      </div>
    </div>
  );
}
...
function OrdersView({ orders, onBack, onViewDetails, isMobile }: { orders: Order[], onBack: () => void, onViewDetails: (order: Order) => void, isMobile?: boolean }) {
  return (
    <div className={`mx-auto px-4 py-8 fade-in ${isMobile ? 'max-w-full' : 'max-w-5xl'}`}>
          ...
          {orders.map((order) => (
            <div key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-6">
                ...
                {/* Meta Info */}
                <div className={`grid gap-6 mb-6 ${isMobile ? 'grid-cols-1' : 'grid-cols-3'}`}>
...
function OrderConfirmationView({ order, onHome, onViewOrders, isMobile }: { order: Order, onHome: () => void, onViewOrders: () => void, isMobile?: boolean }) {
  const subtotal = order.items.reduce((acc, item) => acc + (item.price * item.quantity), 0);
  const shippingCost = order.total - subtotal;
  
  return (
    <div className={`mx-auto px-4 py-12 fade-in ${isMobile ? 'max-w-full' : 'max-w-4xl'}`}>
       ...
       {/* Status Cards */}
       <div className={`grid gap-4 mb-8 ${isMobile ? 'grid-cols-1' : 'grid-cols-3'}`}>
...
function PaymentView({ cart, checkoutData, onBack, onConfirmPayment, isMobile }: { cart: CartItem[], checkoutData: CheckoutData, onBack: () => void, onConfirmPayment: () => void, isMobile?: boolean }) {
  const subtotal = cart.reduce((acc, item) => acc + (item.price * item.quantity), 0);
  const shippingCost = checkoutData.deliveryType === 'delivery' ? 5.00 : 0.00;
  const total = subtotal + shippingCost;
  ...
  return (
    <div className={`mx-auto px-4 py-8 fade-in ${isMobile ? 'max-w-full' : 'max-w-6xl'}`}>
       <button 
        onClick={onBack}
...
function CheckoutView({ cart, onBack, onNext, isMobile }: { cart: CartItem[], onBack: () => void, onNext: (data: CheckoutData) => void, isMobile?: boolean }) {
    ...
    <div className={`mx-auto px-4 py-8 fade-in ${isMobile ? 'max-w-full' : 'max-w-6xl'}`}>
      ...
      <div className={`grid gap-8 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-3'}`}>
        ...
        <div className="lg:col-span-1">
           <div className={`bg-white rounded-2xl shadow-sm p-6 ${isMobile ? '' : 'sticky top-24'}`}>
...
function AIEditor({ imageSrc, onSave, onCancel, isMobile }: { imageSrc: string, onSave: (src: string) => void, onCancel: () => void, isMobile?: boolean }) {
  ...
  return (
    <div className={isMobile ? "absolute inset-0 z-50 flex flex-col bg-gray-50 top-8" : "fixed inset-0 bg-gray-50 z-[100] flex flex-col"}>
       {/* Header */}
       <header className={`bg-white border-b border-gray-200 flex items-center justify-between shadow-sm flex-shrink-0 ${isMobile ? 'px-4 py-3' : 'px-6 py-4'}`}>
          <div className="flex items-center gap-2">
             <div className={`rounded-lg text-white bg-[#FF7F40] ${isMobile ? 'p-1.5' : 'p-2'}`}>
                <Sparkles className={isMobile ? 'w-4 h-4' : 'w-5 h-5'} />
             </div>
             <h1 className={`font-bold text-gray-800 ${isMobile ? 'text-lg' : 'text-xl'}`}>Editor con IA</h1>
          </div>
          <div className="flex items-center gap-3">
             <button 
                onClick={onCancel}
                className={`text-gray-500 hover:text-gray-700 font-medium transition-colors ${isMobile ? 'text-sm px-2' : 'px-4 py-2'}`}
             >
               Cancelar
             </button>
             <button 
                onClick={() => onSave(currentImage)}
                className={`bg-[#00A9C3] hover:bg-[#008e9b] text-white font-bold rounded-full shadow-md transition-colors flex items-center gap-2 ${isMobile ? 'px-4 py-1.5 text-sm' : 'px-6 py-2'}`}
             >
               <Save className="w-4 h-4" />
               <span className={isMobile ? 'hidden sm:inline' : ''}>Guardar y Usar</span>
               <span className={isMobile ? 'sm:hidden' : 'hidden'}>Guardar</span>
             </button>
          </div>
       </header>
       ...
       <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className={`flex-1 bg-gray-100 flex flex-col items-center justify-center relative overflow-hidden ${isMobile ? 'p-2 pb-16' : 'p-2 md:p-8'}`}>
             
             {/* Main Image Container */}
             <div className={`flex-1 flex items-center justify-center w-full min-h-0 ${isMobile ? '' : 'mb-20'}`}>
               <div className={`bg-white shadow-lg rounded-xl overflow-hidden w-full h-full border border-gray-200 relative flex items-center justify-center ${isMobile ? 'max-h-[55vh]' : ''}`}>
                  <img 
                    src={currentImage} 
                    alt="Current AI Edit" 
          ...
          <div className={`bg-white border-t border-gray-200 flex flex-col shadow-xl z-20 flex-shrink-0 ${isMobile ? 'h-[40%] w-full' : 'w-full h-[300px] md:h-[350px]'}`}>
...
function PhotoEditor({ imageSrc, onSave, onCancel, onRequestAI, isMobile }: { imageSrc: string, onSave: (src: string) => void, onCancel: () => void, onRequestAI: () => void, isMobile?: boolean }) {
  ...
  return (
    <div className={`mx-auto px-4 ${isMobile ? 'py-4' : 'py-8'} fade-in max-w-6xl`}>
       <h1 className={`font-bold text-center text-[#00A9C3] ${isMobile ? 'text-2xl mb-6' : 'text-3xl mb-10'}`}>Editar Foto</h1>

       <div className={`grid grid-cols-1 md:grid-cols-2 ${isMobile ? 'gap-6' : 'gap-8'}`}>
          {/* Left: Preview */}
          <div className="flex flex-col gap-4">
             <div className={`bg-white rounded-2xl shadow-sm ${isMobile ? 'p-3' : 'p-6'}`}>
                <div className="flex items-center gap-2 mb-4 text-[#FF7F40] font-bold">
                   <Sparkles className="w-5 h-5" />
                   <h3>Vista Previa</h3>
                </div>
                
                <div className={`bg-gray-100 rounded-xl overflow-hidden flex items-center justify-center ${isMobile ? 'h-[40vh]' : 'aspect-[4/5]'}`}>
                   <img 
                    src={imageSrc} 
                    alt="Editing" 
          ...
          <div className="flex flex-col gap-6">
             ...
             {/* Sliders */}
             <div className={`bg-white rounded-2xl shadow-sm space-y-8 ${isMobile ? 'p-4' : 'p-6'}`}>
                {/* Brightness */}
                <CustomSlider 
                  label="Brillo" 
...
function LandingPage({ onProductClick, isMobile }: { onProductClick: (p: Product) => void, isMobile?: boolean }) {
  return (
    <>
      {/* --- HERO SECTION --- */}
      <section className={`relative bg-gradient-to-r from-cyan-50 to-orange-50 flex items-center justify-center ${isMobile ? 'py-8 min-h-[400px]' : 'py-20 min-h-[600px]'} px-4`}>
        <div className={`bg-white rounded-3xl shadow-xl text-center w-full z-10 transform hover:scale-[1.01] transition-transform duration-500 border border-gray-50 ${isMobile ? 'p-6 max-w-sm' : 'p-10 md:p-16 max-w-2xl'}`}>
          
          <h1 className={`font-bold mb-2 tracking-tight ${isMobile ? 'text-4xl' : 'text-5xl md:text-6xl'}`}>
      ...
      <section className="py-20 px-4 md:px-12 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className={`grid gap-8 ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4'}`}>
...
function ProductDetailView({ product, onBack, onCustomize, isMobile }: { product: Product; onBack: () => void, onCustomize: () => void, isMobile?: boolean }) {
  const thumbnails = product.thumbnails || [product.imgSrc, product.imgSrc];

  return (
    <div className={`mx-auto px-4 md:px-12 py-8 fade-in bg-white min-h-[80vh] ${isMobile ? 'max-w-full' : 'max-w-7xl'}`}>
      <button 
        onClick={onBack}
      ...
      <div className={`grid gap-12 lg:gap-20 ${isMobile ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'}`}>
...
function ProductCard({ product, onClick, isMobile }: ProductCardProps) {
  return (
    <div 
      onClick={onClick}
    ...
    >
      {/* Image Area */}
      <div className={`${isMobile ? 'h-40 p-3' : 'h-80 p-5'} ${product.bgColor} flex items-center justify-center`}>
        {/* Frame Effect Container */}
        <div className={`relative w-full h-full shadow-lg rotate-1 group-hover:rotate-0 transition-transform duration-300 bg-white shadow-gray-400/30 ${isMobile ? 'p-1' : 'p-2'}`}>
      ...
      {/* Content Area */}
      <div className={`flex flex-col flex-1 text-center ${isMobile ? 'p-3' : 'p-6'}`}>
        <h3 className={`font-bold text-gray-900 uppercase mb-3 leading-tight group-hover:text-[#FF7F40] transition-colors ${isMobile ? 'text-xs' : 'text-sm'}`}>
          {product.title}
        </h3>
        ...
        <div className="flex items-end justify-center gap-2 mt-auto">
          ...
          <span className={`font-bold text-[#FF7F40] ${isMobile ? 'text-lg' : 'text-xl'}`}>
            {product.price}
          </span>
        </div>
      </div>
